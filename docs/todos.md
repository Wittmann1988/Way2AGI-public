...
- [ ] **Orchestrierungs-Server auf npu-node** — Lokaler Server der Tasks automatisch uebernimmt, orchestriert, und Modelle aller Netzwerk-Geraete nutzt. Muss mit Network Agent verbunden sein (weiss welche Modelle verfuegbar, sorgt dass alle erreichbar bleiben). Endpunkt: http://YOUR_NPU_NODE_IP:8200 (oder aehnlich). Komponenten:
...
- [ ] **Model-Evaluierung aller lokalen Modelle** — Jedes Modell auf jedem Node systematisch bewerten: Latenz, Qualitaet (1-5), Token/s, Staerken/Schwaechen pro Task-Typ (Triage, Code, Summarization, Reasoning, Classification). Ergebnisse in DB speichern damit Orchestrator weiss welches Modell wann zu nutzen ist. Modelle: lfm2:24b, smallthinker:1.8b, qwen3:1.7b, nemotron (Inference Node), step-3.5-flash (Desktop), llama4-maverick (Cloud).
...
- [ ] **the operator's 6 Regeln implementieren** — Roundtable: Wie werden die 6 Regeln (Selbstbeobachtung, Fehler registrieren, Selbstreflexion, Forschung, Training, Weiterentwicklung) + GoalGuard-Prioritaet als Code umgesetzt? Elias Memory braucht eigene Kategorien: rules, todos, milestones, endgoal.
- [ ] **npu-node Daemon deployen** — Ollama installiert + 3 Modelle (lfm2, smallthinker, qwen3:1.7b). Daemon noch nicht gestartet.
- [ ] **LFM2 in CAPABILITY_MAP** — Auf Inference Node, Desktop, npu-node verfuegbar. CAPABILITY_MAP updaten (T1 Tier).
...
- [ ] **npu-node Keep-Alive** — Windows Sleep verhindern (SetThreadExecutionState)
...
- [x] npu-node SSH eingerichtet
- [x] Sessions Repo als zentrale Doku + Task Queue
- [x] Desktop Daemon CONTROLLER_URL Bug gefixt (.129:8050 -> .21:8050)
- [x] LFM2:24b auf Inference Node, Desktop, npu-node gepullt
- [x] npu-node Ollama installiert + 3 Modelle (lfm2, smallthinker, qwen3:1.7b)
- [x] the operator's 6 Regeln im Elias Memory gespeichert (core, importance 1.0)
