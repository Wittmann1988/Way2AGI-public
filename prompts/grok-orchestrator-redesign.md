...
Wir bauen Way2AGI — ein selbstverbesserndes Multi-Node KI-System. Aktuell haben wir einen **zentralen Orchestrator** auf dem Inference Node der alle Tasks routet. Das Problem: Single Point of Failure, und der zentrale Server muss alle Modelle auf allen Nodes kennen.
...
```
            ...
            │                 │                  │
   ┌────────▼────────┐ ┌─────▼──────┐  ┌───────▼───────┐
   │ Desktop RTX5090 │ │ npu-node    │  │ S24 Ultra     │
...
### Probleme
1. Inference Node muss ALLE Modelle auf ALLEN Nodes kennen
2. Routing-Logik ist hardcoded (model_map dict)
3. Wenn Inference Node offline → ganzes System tot
4. Einfache Tasks werden an zu grosse Modelle geroutet
5. Keine echte dezentrale Entscheidungsfindung

## the operator's Vision: Micro-Orchestrator pro Geraet
...
Der **zentrale Orchestrator** (Inference Node) spricht dann nur noch die Geraete-Orchestratoren an, nicht mehr direkt die Modelle.
...
- Redundanz: Was wenn Inference Node offline geht?
...
- Wir haben: Nemotron-30B + Nemotron-4B-Draft auf Inference Node (31 tok/s)
...
- Chef (120B Cloud) + Reasoner (30B Inference Node) + Researcher (Cloud) + Archivist (Memory, lokal)

## Unsere Hardware

| Node | Hardware | RAM | GPU | Modelle | Latenz |
|------|----------|-----|-----|---------|--------|
| Inference Node | ARM, MAXN | 64 GB shared | Orin GPU | 19 + SpecDec | 1ms lokal |
| Desktop | i9, RTX 5090 | 64 GB + 32 GB VRAM | RTX 5090 | 22 | 30ms |
| npu-node | Intel, NPU | 16 GB | Phi Silica NPU | 4 | 19ms |
